  ...
  function findProjectName(path) {
    ...
    if (typeof meta['mod-project-name'] !== 'undefined') {
      ...
      if (projectIndex >= 0) {
        ...
        modulus.io.print('Selecting ' + meta['mod-project-name'].data);
...
project._packageProject = function(dir, includeModules, cb) {
  ...
  if(!includeModules) {
    var foundbundledDependencies = false;
    
    // Try and read package file for bundledDependencies if it exists add all 
    // folders/files to the ignore file unless they exist in the bundledDependencies
    try{
      var packageFile = require(path.join(dir, 'package.json'));
      var node_modules = fs.readdirSync(path.join(dir, 'node_modules'));

      if(packageFile.bundledDependencies && packageFile.bundledDependencies.length){
        // Filter out folders/files that exist in bundledDependencies
        var ignoreModules = node_modules.filter(function(n) {
          return packageFile.bundledDependencies.indexOf(n) == -1;
        });

        // Add each file to ignore file
        ignoreModules.forEach(function(f){
          fs.appendFileSync(path.join(dir, '.modulus-base-ignore'),path.join('node_modules',f)+'\n');
        })
